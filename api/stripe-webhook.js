...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // Isolation multi-sites : ce webhook ne traite que les paiements Taxi RF
      if (meta.business !== 'taxirf') {
        return res.status(200).json({ received: true, skipped: true, reason: 'not taxirf business' });
      }
